...
public class VideoService {
    ...
    public void addVideoLike(Long videoId, Long userId) {
        Video video = videoDao.getVideoById(videoId);//根据视频id获取视频信息
        if (video == null) {
    ...
    public Map<String, Object> getVideoLikes(Long videoId, Long userId) {
        Long count = videoDao.getVideoLikes(videoId);//查询所有用户的点赞数 量
        VideoLike videoLike = videoDao.getVideoLikeByVideoIdAndUserId(videoId, userId);//判断当前用户是否对该视频点赞
    ...
    public PageResult<VideoComment> pageListVideoComments(Integer size, Integer no, Long videoId) {
        Video video = videoDao.getVideoById(videoId);//根据视频Id查询视频信息
        if (video == null) {
            throw new ConditionException("非法视频！");
        }
        ...
        Integer total = videoDao.pageCountVideoComments(params);//查询出评论的总数
